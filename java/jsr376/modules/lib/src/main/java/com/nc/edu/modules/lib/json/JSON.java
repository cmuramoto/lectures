package com.nc.edu.modules.lib.json;
...
public final class JSON {
	...
	static <T> T doParse(TypeReference<? extends T> type, InputStream in) throws IOException {
		return mapper().readValue(in, type);
	}
...
//					new CommonsModule() };
	...
	public static ObjectMapper newMapper(JsonInclude.Include include, int mode) {
		var rv = JsonMapper.builder().configure(MapperFeature.USE_BASE_TYPE_AS_DEFAULT_IMPL, true).build();
		...
		if (mods != null && mods.length > 0) {
			for (var m : mods) {
				if (m.getClass().getName().equals("com.fasterxml.jackson.datatype.jsr310.JavaTimeModule")) {
					rv.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
					break;
				}
			}
	...
	public static <T> T parse(TypeReference<? extends T> tr, byte[] chunk) {
		try {
			return mapper().readValue(chunk, tr);
		} catch (IOException e) {
			throw new UncheckedIOException(e);
		}
	}

	public static <T> T parse(TypeReference<? extends T> type, ClassLoader cl, String resource) {
		return parse(type, cl.getResource(resource));
	}

	public static <T> T parse(TypeReference<? extends T> type, InputStream in) {
		try {
			return mapper().readValue(in, type);
		} catch (IOException e) {
			throw new UncheckedIOException(e);
	...
	public static <T> T parse(TypeReference<? extends T> type, Path path) {
		try (var is = Files.newInputStream(path)) {
			return doParse(type, is);
		} catch (IOException e) {
			throw new UncheckedIOException(e);
	...
	public static <T> T parse(TypeReference<? extends T> type, Reader in) {
		try {
			return mapper().readValue(in, type);
		} catch (IOException e) {
			throw new UncheckedIOException(e);
	...
	public static <T> T parse(TypeReference<? extends T> tr, String s) {
		try {
	...
	public static <T> T parse(TypeReference<? extends T> type, URL resource) {
		try (var is = resource.openStream()) {
			return doParse(type, is);
		} catch (IOException e) {
			throw new UncheckedIOException(e);
	...
	public static <T> T parseResource(TypeReference<? extends T> type, String resource) {
		return parse(type, Thread.currentThread().getContextClassLoader().getResource(resource));
	}
	...
	public static synchronized void reloadModules(Module... modules) {
		mods = modules;
		m = null;
	}
